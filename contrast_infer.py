...
def parse_args():
    ...
    if args.crf:
        args.crf_list = list()
        for t in args.crf_t:     # 3,5,10
            for alpha in args.crf_alpha:   #  1, 4, 10, 24
    ...
    for i, image in enumerate(multi_scale_flipped_image_list):
        with torch.no_grad():
            ...
            if args.network_type in ['eps', 'contrast']:
                ...
                cam_fg = cam[:-1]
                cam_fg = cam_fg.cpu().numpy() * label.reshape(args.num_classes-1, 1, 1)

                if i % 2 == 1:
                    cam_fg = np.flip(cam_fg, axis=-1)
                cam_list.append(cam_fg)
...
def infer_cam_mp(process_id, image_ids, label_list, cur_gpu, num_class=21):
    ...
    model = getattr(importlib.import_module(args.network), 'Net')(args.num_classes)
    ...
    for i, (img_id, label) in enumerate(zip(image_ids, label_list)):
        ...
        if args.network_type in ['eps', 'contrast']:
            cam_np = np.array(cam_list) # only fg
...
def main_mp():
    ...
    for idx, process_id in enumerate(range(n_total_processes)):
        proc = Process(target=infer_cam_mp,
                       args=(process_id, sub_image_ids[idx], sub_label_list[idx], gpu_list[idx], args.num_classes))
